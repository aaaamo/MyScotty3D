...
std::pair<Ray, float> Camera::sample_ray(RNG &rng, uint32_t px, uint32_t py)
{
	...
	float h = 2.0f * tan(vertical_fov / 2.0f * float(M_PI) / 180.0f);
	float w = aspect_ratio * h;
	float wx = sensor_pixel.x * w / film.width - w / 2.0f;
	float wy = sensor_pixel.y * h / film.height - h / 2.0f;

	// Build ray:
	Ray ray;
	ray.point = Vec3();				// ray should start at the origin
	ray.dir = Vec3(wx, wy, -1.0f);	// TODO: compute from sensor plane position
