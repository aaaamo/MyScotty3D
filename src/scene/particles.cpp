...
bool Particles::Particle::update(const PT::Aggregate &scene, Vec3 const &gravity, const float radius, const float dt)
{
	...
	if (trace.hit)
	{
		...
		float cos_theta = dot(v_norm, normal);
		float t = (trace.distance - radius / std::abs(cos_theta)) / v_size;
		...
		if (t < dt)
		{
			...
			{
				velocity = velocity - 2 * dot(normal, velocity) * normal;
				return update(scene, gravity, radius, dt);
			}
		}
	}
