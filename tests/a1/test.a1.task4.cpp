	...
	float out_depth, Spectrum out_color)
{
	...
	if (fb.color_at(0, 0, 0) != BlankColor || fb.color_at(1, 0, 0) != BlankColor || fb.color_at(0, 1, 0) != BlankColor)
		throw Test::error("Pixel color other than upper right was written when running pipeline.");
	if (fb.depth_at(0, 0, 0) != BlankDepth || fb.depth_at(1, 0, 0) != BlankDepth || fb.depth_at(0, 1, 0) != BlankDepth)
		throw Test::error("Pixel color other than upper right was written when running pipeline.");
	...
	if (Test::differs(fb.color_at(1, 1, 0), out_color) || fb.depth_at(1, 1, 0) != out_depth)
	{
		...
		if (fb.depth_at(1, 1, 0) != out_depth)
		...
		if (Test::differs(fb.color_at(1, 1, 0), out_color))
			params += "  (DOES NOT MATCH)\n";
		else
			params += "  (matches)\n";
...
Test test_a1_task4_blend_add_2("a1.task4.blend.add.2", []()
							   {
								   float interpolated_depth, vertex_depth;

								   fragment_depth_to_interpolated_and_vertex_depth(0.5f, &interpolated_depth, &vertex_depth);

								   if (Test::differs(interpolated_depth, 0.5f))
									   throw Test::error("triangle depth interpolation not working as expected, can't perform this test.");

								   check_fragment_writeback<Pipeline_Blend_Add | Pipeline_Depth_Always>(

									   "[0.1,0.2,0.3] opacity 1 + [0.4, 0.5, 0.6]",

									   vertex_depth, Spectrum(0.1f, 0.2f, 0.3f), 0.5f, // fragment

									   1.0f, Spectrum(0.4f, 0.5f, 0.6f), // fb

									   interpolated_depth, Spectrum(0.45f, 0.6f, 0.75f) // out

								   ); });

Test test_a1_task4_blend_add_3("a1.task4.blend.add.3", []()
							   {
								   float interpolated_depth, vertex_depth;
								   ...
								   check_fragment_writeback<Pipeline_Blend_Add | Pipeline_Depth_Always>(
									   ...
									   vertex_depth, Spectrum(0.1f, 0.2f, 0.4f), 0.25f, // fragment

									   1.0f, Spectrum(0.4f, 0.5f, 0.6f), // fb

									   interpolated_depth, Spectrum(0.425f, 0.55f, 0.7f) // out

								   ); });

Test test_a1_task4_blend_over_2("a1.task4.blend.over.2", []()
								{
									float interpolated_depth, vertex_depth;
									...
									check_fragment_writeback<Pipeline_Blend_Over | Pipeline_Depth_Always>(
										...
										interpolated_depth, Spectrum(0.25f, 0.35f, 0.45f) // out

									);
								});

Test test_a1_task4_blend_over_3("a1.task4.blend.over.3", []()
								{
									float interpolated_depth, vertex_depth;
									...
									check_fragment_writeback<Pipeline_Blend_Over | Pipeline_Depth_Always>(
										...
										interpolated_depth, Spectrum(0.325f, 0.425f, 0.55f) // out
