...
static std::string to_string(FPClippedVertex const &vert)
{
	std::string ret = "{"
					  " .fb_position = (" +
					  std::to_string(vert.fb_position.x) + ", " + std::to_string(vert.fb_position.y) + ", " + std::to_string(vert.fb_position.z) + ")"
																																				   ", .inv_w = " +
...
static std::string to_string(FPFragment const &frag)
{
	std::string ret = "{"
					  " .fb_position = (" +
					  std::to_string(frag.fb_position.x) + ", " + std::to_string(frag.fb_position.y) + ", " + std::to_string(frag.fb_position.z) + ")";
	ret += ", .attributes = [";
...
static void check_rasterize_triangles(std::string const &desc, std::initializer_list<FlatPipeline::ClippedVertex> const &vertices_, std::initializer_list<FlatPipeline::Fragment> const &expected_)
{
	...
	{
		if (v.fb_position.x < 0.0f || v.fb_position.x > float(Framebuffer::MaxWidth) || v.fb_position.y < 0.0f || v.fb_position.y > float(Framebuffer::MaxHeight) || v.fb_position.z < 0.0f || v.fb_position.z > 1.0f)
		{
	...
	for (uint32_t gi = 0; gi < got.size(); ++gi)
	{
		...
		for (uint32_t i = 0; i < expected.size(); ++i)
		{
			...
			if (!(g.fb_position.x == e.fb_position.x && g.fb_position.y == e.fb_position.y))
				continue;
			...
			if (!std::equal(g.attributes.begin(), g.attributes.end(), e.attributes.begin(), e.attributes.end(), attribute_close))
				continue;
			// check derivatives (approximately) match:
			if (!std::equal(g.derivatives.begin(), g.derivatives.end(), e.derivatives.begin(), e.derivatives.end(), derivative_close))
				continue;
	...
	auto dump_info = [&]()
	{
		...
		for (uint32_t i = 2; i < vertices.size(); i += 3)
		{
			...
			std::cout << "  rasterize_triangle(v[" << i - 2 << "],v[" << i - 1 << "],v[" << i << "]) emitted " << count << " fragments:\n";
			for (auto const &f : got)
			{
...
Test test_a1_task3_raster_top_1("a1.task3.raster.top.1", []()
								{ check_rasterize_triangles(
									  "thin triangle exceeding [2,3]x[1,3] but only covering (2.5,1.5) and (2.5,2.5)",
...
Test test_a1_task3_raster_top_2("a1.task3.raster.top.2", []()
								{ check_rasterize_triangles(
									  "thin triangle exceeding [2,3]x[1,3] but only covering (2.5,1.5) and (2.5,2.5)",
									  {FPClippedVertex{Vec3{0.5f, 2.5f, 0.5f}, 1.0f, {1.0f}},
									   FPClippedVertex{Vec3{2.5f, 2.5f, 0.5f}, 2.0f, {2.0f}},
									   FPClippedVertex{Vec3{0.5f, 0.5f, 0.5f}, 3.0f, {3.0f}}},
									  {
										  FPFragment{Vec3{0.5f, 1.5f, 0.5f}, {1.0f}, {Vec2{0.0f}}},
										  FPFragment{Vec3{0.5f, 2.5f, 0.5f}, {1.0f}, {Vec2{0.0f}}},
...
Test test_a1_task3_raster_top_3("a1.task3.raster.top.3", []()
								{ check_rasterize_triangles(
									  "thin triangle exceeding [2,3]x[1,3] but only covering (2.5,1.5) and (2.5,2.5)",
									  {FPClippedVertex{Vec3{2.5f, 2.5f, 0.5f}, 1.0f, {1.0f}},
									   FPClippedVertex{Vec3{0.5f, 2.5f, 0.5f}, 2.0f, {2.0f}},
									   FPClippedVertex{Vec3{0.5f, 0.5f, 0.5f}, 3.0f, {3.0f}}},
									  {
										  FPFragment{Vec3{0.5f, 1.5f, 0.5f}, {1.0f}, {Vec2{0.0f}}},
										  FPFragment{Vec3{0.5f, 2.5f, 0.5f}, {1.0f}, {Vec2{0.0f}}},
...
Test test_a1_task3_raster_bottom_1("a1.task3.raster.bottom.1", []()
								   { check_rasterize_triangles(
										 "thin triangle exceeding [2,3]x[1,3] but only covering (2.5,1.5) and (2.5,2.5)",
...
Test test_a1_task3_raster_bottom_2("a1.task3.raster.bottom.2", []()
								   { check_rasterize_triangles(
										 "thin triangle exceeding [2,3]x[1,3] but only covering (2.5,1.5) and (2.5,2.5)",
										 {FPClippedVertex{Vec3{2.5f, 0.5f, 0.5f}, 1.0f, {1.0f}},
										  FPClippedVertex{Vec3{2.5f, 2.5f, 0.5f}, 2.0f, {2.0f}},
										  FPClippedVertex{Vec3{0.5f, 0.5f, 0.5f}, 3.0f, {3.0f}}},
										 {
											 FPFragment{Vec3{1.5f, 1.5f, 0.5f}, {1.0f}, {Vec2{0.0f}}},
...
Test test_a1_task3_raster_bottom_3("a1.task3.raster.bottom.3", []()
								   { check_rasterize_triangles(
										 "thin triangle exceeding [2,3]x[1,3] but only covering (2.5,1.5) and (2.5,2.5)",
										 {FPClippedVertex{Vec3{2.5f, 2.5f, 0.5f}, 1.0f, {1.0f}},
										  FPClippedVertex{Vec3{2.5f, 0.5f, 0.5f}, 2.0f, {2.0f}},
										  FPClippedVertex{Vec3{0.5f, 0.5f, 0.5f}, 3.0f, {3.0f}}},
										 {
											 FPFragment{Vec3{1.5f, 1.5f, 0.5f}, {1.0f}, {Vec2{0.0f}}},
										 }); });

// These test cases are assuming that we are using Microsoft's top left rule for vertices that lie on pixel centers

Test test_a1_task3_triangle_1left("a1.task3.triangle.1left", []()
								  {
									  check_rasterize_triangles(
										  ...
										  {FPClippedVertex{Vec3{0.0f, 0.0f, 0.5f}, 1.0f, {1.0f}},
										   ...
										   FPClippedVertex{Vec3{4.0f, 0.0f, 0.0f}, 2.0f, {2.0f}},

										   FPClippedVertex{Vec3{6.0f, 2.0f, 1.0f}, 3.0f, {3.0f}}},

										  {FPFragment{Vec3{1.5f, 0.5f, 0.625f}, {1.0f}, {Vec2{0.0f}}},

										   FPFragment{Vec3{2.5f, 0.5f, 0.5f}, {1.0f}, {Vec2{0.0f}}},

										   FPFragment{Vec3{3.5f, 0.5f, 0.375f}, {1.0f}, {Vec2{0.0f}}},

										   FPFragment{Vec3{4.5f, 1.5f, 0.875f}, {1.0f}, {Vec2{0.0f}}}}

									  );
								  });

Test test_a1_task3_triangle_novertices("a1.task3.triangle.novertices", []()
									   {
										   check_rasterize_triangles(

											   "triangle on endpoints but no points inside",

											   {FPClippedVertex{Vec3{0.5f, 1.5f, 0.6f}, 1.0f, {1.0f}},

												FPClippedVertex{Vec3{1.5f, 0.5f, 0.9f}, 2.0f, {2.0f}},

												FPClippedVertex{Vec3{0.5f, 0.5f, 0.1f}, 3.0f, {3.0f}}},
...
Test test_a1_task3_triangle_someverticeccw("a1.task3.triangle.someverticesccw", []()
										   {
											   check_rasterize_triangles(

												   "triangle on endpoints,1 point inside",

												   {FPClippedVertex{Vec3{1.5f, 0.5f, 0.6f}, 1.0f, {1.0f}},

													FPClippedVertex{Vec3{1.5f, 1.5f, 0.9f}, 2.0f, {2.0f}},

													FPClippedVertex{Vec3{0.5f, 1.5f, 0.1f}, 3.0f, {3.0f}}},

												   {FPFragment{Vec3{0.5f, 1.5f, 0.1f}, {1.0f}, {Vec2{0.0f}}}}

											   );
										   });

Test test_a1_task3_triangle_someverticescw("a1.task3.triangle.someverticescw", []()
										   {
											   check_rasterize_triangles(
...
Test test_a1_task3_triangle_zinterptest("a1.task3.triangle.zinterptest", []()
										{
											check_rasterize_triangles(

												"big triangle test for z interpolation",

												{FPClippedVertex{Vec3{0.5f, 0.5f, 0.6f}, 1.0f, {1.0f}},

												 FPClippedVertex{Vec3{0.5f, 2.5f, 0.1f}, 3.0f, {3.0f}},

												 FPClippedVertex{Vec3{3.5f, 2.5f, 0.9f}, 2.0f, {2.0f}}},

												{FPFragment{Vec3{0.5f, 1.5f, 0.35f}, {1.0f}, {Vec2{0.0f}}},

												 FPFragment{Vec3{1.5f, 1.5f, 0.6167f}, {1.0f}, {Vec2{0.0f}}},

												 FPFragment{Vec3{0.5f, 2.5f, 0.1f}, {1.0f}, {Vec2{0.0f}}},

												 FPFragment{Vec3{1.5f, 2.5f, 0.36667f}, {1.0f}, {Vec2{0.0f}}},

												 FPFragment{Vec3{2.5f, 2.5f, 0.63333f}, {1.0f}, {Vec2{0.0f}}}}

											);
										});

Test test_a1_task3_triangle_degenerate("a1.task3.triangle.degenerate", []()
									   {
										   check_rasterize_triangles(
											   ...
											   {FPClippedVertex{Vec3{0.0f, 0.0f, 0.5f}, 1.0f, {1.0f}},
												...
												FPClippedVertex{Vec3{2.0f, 0.0f, 0.5f}, 3.0f, {3.0f}},

												FPClippedVertex{Vec3{4.0f, 0.0f, 0.5f}, 2.0f, {2.0f}}},
...
Test test_a1_task3_triangle_l2("a1.task3.triangle.l2", []()
							   {
								   check_rasterize_triangles(

									   "triangle only 1 fragment",

									   {FPClippedVertex{Vec3{0.5f, 0.5f, 0.0f}, 1.0f, {1.0f}},

										FPClippedVertex{Vec3{2.5f, 2.5f, 1.0f}, 3.0f, {3.0f}},
...
Test test_a1_task3_triangle_isosceles_down("a1.task3.triangle.isosceles.down", []()
										   {
											   check_rasterize_triangles(

												   "downwards facing isosceles",

												   {FPClippedVertex{Vec3{2.5f, 2.5f, 0.75f}, 1.0f, {1.0f}},

													FPClippedVertex{Vec3{4.5f, 2.5f, 1.0f}, 3.0f, {3.0f}},

													FPClippedVertex{Vec3{3.5f, 0.5f, 0.5f}, 2.0f, {2.0f}}},

												   {FPFragment{Vec3{3.5f, 2.5f, 0.875f}, {1.0f}, {Vec2{0.0f}}},

													FPFragment{Vec3{3.5f, 1.5f, 0.6875f}, {1.0f}, {Vec2{0.0f}}},

													FPFragment{Vec3{2.5f, 2.5f, 0.75f}, {1.0f}, {Vec2{0.0f}}}}

											   );
										   });

Test test_a1_task3_triangle_isosceles_right("a1.task3.triangle.isosceles.right", []()
											{
												check_rasterize_triangles(

													"right facing iscosceles",

													{FPClippedVertex{Vec3{0.5f, 0.5f, 0.75f}, 3.0f, {3.0f}},

													 FPClippedVertex{Vec3{3.5f, 1.5f, 1.0f}, 6.0f, {3.0f}},

													 FPClippedVertex{Vec3{0.5f, 2.5f, 0.5f}, 1.2f, {2.0f}}},

													{FPFragment{Vec3{0.5f, 1.5f, 0.625f}, {3.0f}, {Vec2{0.0f}}},

													 FPFragment{Vec3{1.5f, 1.5f, 0.75f}, {3.0f}, {Vec2{0.0f}}},

													 FPFragment{Vec3{2.5f, 1.5f, 0.875f}, {3.0f}, {Vec2{0.0f}}}}
